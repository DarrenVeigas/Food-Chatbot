...
def add_to_order(parameters: dict, session_id: str):
    ...
    if len(food_items) != len(quantities):
        ...
        if session_id in inprogress:
            current_food_dict = inprogress[session_id]
            current_food_dict.update(new_food_dict)
            inprogress[session_id] = current_food_dict
        else:
            inprogress[session_id] = new_food_dict

        order_str = generic_helper.get_str_from_food_dict(inprogress[session_id])
...
def complete_order(parameters,session_id):
    if session_id not in inprogress:
        fulfillment_text = "I'm having a trouble finding your order. Sorry! Can you place a new order please?"
    else:
        order = inprogress[session_id]
...
def remove_from_order(parameters,session_id):
    ...
    current_order = inprogress[session_id]
...
def track_order(parameters,session_id):
    ...
    stat=db_helper.get_order_status(order_id)
    if stat is not None:
        fulfillment_text=f"The order status for order id :{order_id} is {stat}"
    else:
